...
export default function FastingTimer({ session, initialSeconds, eatingWindowActive = false }) {
  ...
  return (
    <div className="relative py-16 sm:py-24" ref={timerRef}>
      ...
      {/* Info row */}
      <div className="flex items-center justify-between mt-6 px-0 sm:px-2 gap-2">
        <div className="flex items-center gap-2 min-w-0 shrink">
          <motion.div
            ...
            transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
          />
          <span className="text-[10px] sm:text-xs uppercase tracking-[0.15em] sm:tracking-[0.2em] text-white/60 truncate">
            {statusLabel}
          </span>
        </div>

        <span className="text-sm text-white text-center hidden sm:block">
          {session ? (
            isEatingWindow ? (
        ...
        <Link
          to="/log-meal"
          className="whitespace-nowrap text-[10px] sm:text-xs uppercase tracking-[0.1em] sm:tracking-[0.2em] border border-primary-500 text-primary-500 hover:bg-primary-500 hover:text-black px-3 sm:px-5 py-1.5 sm:py-2 transition-all duration-300 shrink-0"
        >
          Log Meal
        </Link>
      </div>
